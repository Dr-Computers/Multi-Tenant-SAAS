...
<script>$(document).ready(function() {
    $("#min, #max").datepicker({
        yearRange: "1900:2024", // Adjust the range as needed
        changeYear: true, // Enable year dropdown
        changeMonth: true, // Enable month dropdown
        maxDate: 0 // Optional: Disallow future dates
    });
});</script>
@endpush --}}
<script>
...
@section('content')
    ...
    <div class="row">
        ...
        <div style="display: flex; gap: 20px; align-items: center; flex-wrap: wrap; margin-bottom: 20px;">
            ...
            <form method="GET" action="{{ route('report.payments.index') }}">
            ...
            <a href="{{ route('report.payments.index') }}" class="btn btn-secondary btn-sm">
                {{ __('Clear') }}
            </a>
        </div>
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <table class="display dataTable cell-border datatbl-advance transaction-table reports-table" data-report-name="Payment Report">
                        <thead>
                        <tr>
                        ...
                        @foreach($payments as $payment)
                            <tr role="row">
                                ...
                                <td>{{ (optional($payment->invoice->properties)->invoice_prefix ?: invoicePrefix()) .optional($payment->invoice)->invoice_id ?? 'N/A' }}</td>
                                <td>{{ optional($payment->invoice->units)->name ?? 'N/A' }}</td>
                                <td>{{ optional(optional(optional($payment->invoice)->units)->activeLease->tenant)->user->first_name ?? 'N/A' }}</td>
