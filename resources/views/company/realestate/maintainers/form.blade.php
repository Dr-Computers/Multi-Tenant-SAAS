@canany(['create maintainer user', 'edit maintainer user'])

    <form
        action="{{ isset($user) ? route('company.realestate.maintainers.update', $user->id) : route('company.realestate.maintainers.store') }}"
        ...
        <div class="modal-body">
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="form-group">
                        <label for="type" class="form-label">Type</label><x-required></x-required>
                        <select name="type" value="{{ old('trn_no', $user->name ?? '') }}" class="form-control "
                            autocomplete="off" required>
                            @foreach ($types ?? [] as $type)
                                <option
                                    {{ isset($user) && $user->maintainer && $user->maintainer->type_id == $type->id ? 'selected' : '' }}
