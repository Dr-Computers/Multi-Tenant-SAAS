{{ Form::open([
    'route' => isset($role) ? ['company.hrms.roles.update', $role->id] : 'company.hrms.roles.store',
        ...
        <div class="col-md-12">
            <div class="form-group">
                @if (!empty($permissions))
                    ...
                    @if ($permissions->groupBy('section')->count())
                    <div class="mb-2">
                        <input type="checkbox" id="checkall" class="form-check-input">
                        <label for="checkall" class="form-check-label"><strong>{{ __('Check All') }}</strong></label>
                    </div>
                    @endif
                    @forelse ($permissions->groupBy('section') ?? [] as $section => $permission)
                        <div class="row mb-2">
                            <div class="col-lg-12">
                                <h6>{{ $section }}</h6>
                            </div>
                    ...
                    <div class="row mb-2">
                        <div class="col-lg-12">
                            <small class="text-center">No Permissions Found</small>
                        </div>
                    </div>
                    @endforelse
                @endif
