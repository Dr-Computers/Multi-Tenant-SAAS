...
@section('action-btn')
    <div class="d-flex">
        <a  href="{{ route('company.realestate.tenants.index') }}"  title="{{ __('Back to tenants') }}" class="btn btn-sm btn-primary me-2">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <ul class="nav nav-tabs" id="tenantTabs">
                        ...
                        <li class="nav-item">
                            <a class="nav-link {{ $tab == 'properties' ? 'active' : '' }}"
                                href="{{ route('company.realestate.tenants.show', $tenant->id) }}?tab=properties">Leasing Properties</a>
                        </li>
                        <li class="nav-item">
                        ...
                        <li class="nav-item">
                            <a class="nav-link {{ $tab == 'requests' ? 'active' : '' }}"
                                href="{{ route('company.realestate.tenants.show', $tenant->id) }}?tab=requests">Requests for
                                Approval</a>
                        </li>
                        <li class="nav-item">
                        ...
                        <li class="nav-item">
                            <a class="nav-link {{ $tab == 'activity_logs' ? 'active' : '' }}"
                                href="{{ route('company.realestate.tenants.show', $tenant->id) }}?tab=activity_logs">Activity Log</a>
                        </li>
                    </ul>
                    ...
                    <div class="tab-content p-4 border border-top-0 rounded-bottom">
                        @if ($tab == 'overview')
                            @include('company.realestate.tenants.partials._overview', ['tenant' => $tenant])
                        @elseif($tab == 'properties')
                            @include('company.realestate.tenants.partials._properties', ['tenant' => $tenant])
                        @elseif($tab == 'requests')
                            @include('company.realestate.tenants.partials._requests', ['tenant' => $tenant])
                        @elseif($tab == 'settlements')
                            @include('company.realestate.tenants.partials._settlements', [
                                'tenant' => $tenant,
                            ])
                        @elseif($tab == 'documents')
                            @include('company.realestate.tenants.partials._documnets', ['tenant' => $tenant])
                        @elseif($tab == 'activity_logs')
                            @include('company.realestate.tenants.partials._activity_logs', ['tenant' => $tenant])
                        @endif
                    </div>
                </div>
            </div>
