...
{{--         
        ...
        <div style="display: flex; gap: 20px; align-items: center; flex-wrap: wrap; margin-bottom: 20px;">
            ...
            <form method="GET" action="{{ route('report.expenses.index') }}">
            ...
            <a href="{{ route('report.expenses.index') }}" class="btn btn-secondary btn-sm">
                {{ __('Clear') }}
            </a>
        </div>
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <table class="display dataTable cell-border datatbl-advance-expense" data-report-name="Expense Report">
                        <thead>
                            <tr>
                                {{-- <th>{{__('Tenant')}}</th> --}}
                                <th>{{__('Date')}}</th>
                        ...
                        {{-- <tfoot>
                            <tr>
                                <th colspan="8" style="text-align: right;">{{ __('Total') }}</th>
                                <th>{{ priceFormat($totalBaseAmount) }}</th>
                                <th>{{ priceFormat($totalVatAmount) }}</th>
                                <th>{{ priceFormat($grandTotalAmount) }}</th>
                            </tr>
                        </tfoot> --}}
                        ...
                        <tfoot>
                            <tr>
                                ...
                                <th colspan="8"></th>
                                <th colspan="1" id="amount">{{ priceFormat($baseAmount) }}</th> <!-- Total for Amount -->
                                <th colspan="1" id="vat">{{ priceFormat($vatAmount) }}</th>    <!-- Total for VAT -->
                                <th colspan="1" id="total">{{ priceFormat($totalAmount) }}</th>  <!-- Total for Total -->
                            </tr>
                        </tfoot>
                    </table>
                    <!-- Pagination links -->
