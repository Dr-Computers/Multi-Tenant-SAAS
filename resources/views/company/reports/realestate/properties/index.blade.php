...
@section('content')
    ...
    <div class="row">
        ...
        <div style="display: flex; gap: 20px; align-items: center; flex-wrap: wrap; margin-bottom: 20px;">
            ...
            <form method="GET" action="{{ route('report.properties.index') }}">
            ...
            <a href="{{ route('report.properties.index') }}" class="btn btn-secondary btn-sm">
                {{ __('Clear') }}
            </a>
        </div> 
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <a href="{{ route('report.properties.download') }}">
                        <button class="btn btn-primary btn-sm" style="margin-bottom: 20px;">Download</button>
                        </a>
                    <table class="display dataTable cell-border datatbl-advanc">
                        <thead>
                        <tr>
                                    ...
                                    @if($property->units->isNotEmpty())
                                        <ul>
                                            @foreach($property->units as $unit)
                                                <li>{{ $unit->name }}</li> <!-- Adjust according to your unit field -->
                                            @endforeach
                                        </ul>
                                    @else
                                        {{ __('No Units') }}
