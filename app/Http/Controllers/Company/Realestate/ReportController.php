...
class ReportController extends Controller
{
    public function invoiceIndex(Request $request)
    {
        if (\Auth::user()->can('view invoices report')) {
            $filterProperty = Property::where('parent_id', parentId())->get();
            ...
            $invoicesQuery = Invoice::where('parent_id', parentId())
            ...
            return view('reports.invoices.index', compact('invoices', 'totalAmount', 'filterProperty'));
        } else {
    ...
    //         // Now you can pass payments to the view
    //         $totalAmount = $payments->sum('amount'); // Assuming the column for payment amount is named 'amount'
    //         return view('reports.deposit.index', compact('payments', 'totalAmount'));
    //     } else {
    //         return redirect()->back()->with('error', __('Permission Denied!'));
    //     }
    // }
    public function depositPaymentIndex(Request $request)
    {
        ...
        if (\Auth::user()->can('view payments report')) {
            ...
            return view('reports.deposit.index', compact('payments', 'totalAmount', 'tenants', 'properties', 'units'));
        } else {
            return redirect()->back()->with('error', __('Permission Denied!'));
        }
    }
    ...
    public function TransactionsIndex(Request $request)
    {
        ...
        if (\Auth::user()->can('view payments report')) {
            ...
            return view('reports.transactions.index', compact('transactions', 'bankAccounts', 'closingBalance', 'transactionAmount', 'openingBalance'));
        } else {
            return redirect()->back()->with('error', __('Permission Denied!'));
        }
    }
        ...
        if (\Auth::user()->can('view cheques report')) {
            ...
            if (\Auth::user()->type == 'tenant') {
                ...
                $invoices = Invoice::where('property_id', $tenant->property)
                    ->where('unit_id', $tenant->unit)->where('parent_id', parentId())
                    ->orderBy('created_at', 'desc') // Order by created_at in descending order
                    ->get();

                // $chequeDetails = CheckDetail::whereIn('invoice_id', $invoices->pluck('id'))->get();
                $chequeDetails = CheckDetail::whereIn('invoice_id', $invoices->pluck('id'));
            } else if (\Auth::user()->type == 'propertyowner') {
                ...
                //     ->get();
                // $chequeDetails = CheckDetail::whereIn('invoice_id', $invoices->pluck('id'))->get();
                // $chequeDetails = CheckDetail::with('tenant')->get();
                $chequeDetails = CheckDetail::with('tenant');
            }
            // if ($request->has('property') && $request->property) {
            ...
            return view('reports.cheques.index', compact('chequeDetails', 'totalAmount', 'filterProperty', 'filterTenant'));
        } else {
            return redirect()->back()->with('error', __('Permission Denied!'));
        }
    }
    ...
    public function expenseIndex(Request $request)
    {
        if (\Auth::user()->can('view expenses report')) {
            ...
            return view('reports.expenses.index', compact('expenses', 'totalAmount', 'vatAmount', 'baseAmount', 'filterProperty', 'filterTenant'));
        } else {
            return redirect()->back()->with('error', __('Permission Denied!'));
        }
    }
    ...
    public function tenantsIndex(Request $request)
    {
        if (\Auth::user()->can('view tenants report')) {
            ...
            return view('reports.tenants.index', compact('tenants', 'filterTenant'));
        } else {
            return redirect()->back()->with('error', __('Permission Denied!'));
        }
    }
    ...
    //         $properties = Property::with('units')->where('parent_id', parentId())->where('is_active', 1)->paginate(10);
    ...
    //         return view('reports.properties.index', compact('properties', 'totalProperties'));
    //     } else {
    //         return redirect()->back()->with('error', __('Permission Denied!'));
    //     }
    // }
...

    {
        if (\Auth::user()->can('view properties report')) {
            ...
            $filterProperty = Property::where('parent_id', parentId())->get();
            // Base Query
            $query = Property::with('units')->where('parent_id', parentId())->where('is_active', 1);
            ...
            return view('reports.properties.index', compact('properties', 'totalProperties', 'filterProperty'));
        } else {
            return redirect()->back()->with('error', __('Permission Denied!'));
        }
    }
    ...
    public function unitsIndex(Request $request)
    {
        if (\Auth::user()->can('view units report')) {
            ...
            $units = PropertyUnit::where('parent_id', parentId());
            if ($request->has('property') && $request->property) {
            ...
            return view('reports.units.index', compact(
                'units',
                'totalUnits',
                'tenants',
                'totalAmount',
    ...
    //         $unitsQuery = PropertyUnit::where('parent_id', parentId()); // Start with the base query
    ...
    //         return view('reports.units.index', compact('units', 'totalUnits', 'tenants'));
    //     } else {
    //         return redirect()->back()->with('error', __('Permission Denied!'));
    //     }
    // }
    ...
    public function view($id)
    {
        ...
        return view('reports.units.view', compact('unit', 'invoices', 'tenant', 'checkDetails', 'securityDepositPayments'));
    }
    ...
    public function maintainersIndex(Request $request)
    {
        if (\Auth::user()->can('view maintainer report')) {
            ...
            return view('reports.maintainers.index', compact(
                'maintainers',
                'filterProperty',
                'filterTenant'
            ));
    ...
    public function  maintenancesIndex(Request $request)
    {
        if (\Auth::user()->can('manage maintenance request')) {
            ...
            } elseif (\Auth::user()->type == 'propertyowner') {
                $maintenanceRequests = MaintenanceRequest::whereIn('property_id', getOwnerPropertyIds())
                    ->get();
            } else {
                $filterProperty = Property::where('parent_id', parentId())->get();
                $filterTenant = Tenant::with('user')
            ...
            return view('reports.maintenance.index', compact('maintenanceRequests', 'totalRequests', 'filterProperty', 'filterTenant'));
        } else {
            return redirect()->back()->with('error', __('Permission Denied!'));
        }
    }
    ...
    //     return view('reports.profit_loss.index', compact('rentalIncomeData', 'additionalIncomeData', 'totalExpenses', 'expensesSummary', 'totalIncome', 'netProfitLoss', 'startDate', 'endDate'));
    // }
    public function profitLossIndex(Request $request)
...

    {
        ...
        $invoicePayments = InvoicePayment::select('payment_for', \DB::raw('SUM(amount) as total_amount'))
            ->groupBy('payment_for')
            ->get();



        $totalRentPayment = InvoicePayment::where('payment_for', 'rent')
            ->sum('amount');
        ...
        $totalSecurityDepositPayment = InvoicePayment::where('payment_for', 'security_deposit')
            ->sum('amount');


        // Query for total of all other payments excluding 'rent' and 'security_deposit'
        $totalOtherPayment = InvoicePayment::whereNotIn('payment_for', ['rent', 'security_deposit'])
            ->sum('amount');
        ...
        $expenseTypes = Type::where('type', 'expense')
        ...
        return view('reports.profit_loss.index', compact('totalIncome', 'invoicePayments', 'totalRentPayments', 'totalSecurityDepositPayments', 'totalOtherPayments', 'liabilityExpenses', 'taxExpenses', 'operationalExpenses', 'propertyExpenses', 'expenseTypes', 'expenseSummaries', 'totalExpenses'));
    }
    ...
    //     return view('reports.balance_sheet.index', compact('assetsSummary', 'liabilitiesSummary', 'totalAssets', 'totalLiabilities', 'equity'));
    // }
    public function balanceSheetIndex()
    {
        // Fetch all bank accounts with their closing balances
        $bankAccounts = BankAccount::select('id', 'account_name', 'account_type', 'closing_balance')->get();
        ...
        // ]);
        return view('reports.balance_sheet.index', compact('bankAccounts', 'assetsData', 'equity', 'liabilitiesData', 'totalAssets', 'totalLiabilities', 'depositLiability'));
    }
    ...
    public function leaseExpiryReport(Request $request)
    {
        ...
        $filterTenant = Tenant::with('user')
            ...
            ->select('tenants.*', 'users.first_name', 'users.last_name')
            ->get();
        $query = Lease::with(['tenant', 'unitLease.properties'])
        ...
        return view('reports.lease_expiry.index', compact(
            'leases',
            'filterProperty',
            'filterTenant'
        ));
    ...
    public function fireandsafetyExpiryReport(Request $request)
    {
        ...
        $filterTenant = Tenant::with('user')
            ...
            ->select('tenants.*', 'users.first_name', 'users.last_name')
            ->get();
        $results = Property::where('fire_safety_expiry_date', '>=', now())->orderBy('fire_safety_expiry_date', 'asc')->get();
        ...
        if ($request->has('start_month') && $request->start_month && $request->has('end_month') && $request->end_month) {
            ...
            $results = $results->where('fire_safety_expiry_date', 'like', $request->start_month . '%');
        } else if ($request->has('end_month') && $request->end_month) {
            $results = $results->where('fire_safety_expiry_date', 'like', $request->end_month);
        }

        if (\Auth::user()->type === 'propertyowner') {
            $results = $results->whereIn('id', getOwnerPropertyIds());
        }
        return view('reports.fireandsafety_expiry.index', compact(
            'results',
            'filterProperty',
            'filterTenant'
        ));
    ...
    public function insuranceExpiryReport(Request $request)
    {
        ...
        $filterTenant = Tenant::with('user')
            ...
            ->select('tenants.*', 'users.first_name', 'users.last_name')
            ->get();
        $results = Property::where('insurance_expiry_date', '>=', now())->orderBy('insurance_expiry_date', 'asc')->get();
        if ($request->has('property') && $request->property) {
            $results = $results->where('id', $request->property);
        }
        if ($request->has('start_month') && $request->start_month && $request->has('end_month') && $request->end_month) {
            ...
            $results = $results->where('insurance_expiry_date', 'like', $request->start_month . '%');
        } else if ($request->has('end_month') && $request->end_month) {
            $results = $results->where('insurance_expiry_date', 'like', $request->end_month);
        }

        if (\Auth::user()->type === 'propertyowner') {
            $results = $results->whereIn('id', getOwnerPropertyIds());
        }
        return view('reports.insurance_expiry.index', compact(
            'results',
            'filterProperty',
            'filterTenant'
        ));
    ...
    public function invoiceOutstandingReport(Request $request)
    {
        if (\Auth::user()->can('view properties report')) {
            ...
            return view('reports.properties.outstanding', compact('properties', 'filterProperty'));
        } else {
            return redirect()->back()->with('error', __('Permission Denied!'));
        }
    }
    ...
    public function rentCollectionSummaryReport(Request $request)
    {
        if (\Auth::user()->can('view rent collection')) {
            ...
            return view('reports.rent_collection_summary.index', compact('invoices', 'totalAmount', 'filterProperty', 'filterTenant', 'paid', 'balance'));
        } else {
            return redirect()->back()->with('error', __('Permission Denied!'));
        }
    }
