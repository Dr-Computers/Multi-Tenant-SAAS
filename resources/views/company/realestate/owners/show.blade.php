...
@section('action-btn')
    <div class="d-flex">
        <a href="{{ route('company.realestate.owners.index') }}" title="{{ __('Back to Owners') }}"
                ...
                <div class="card-body">
                    @can('owner user details')
                        ...
                        <ul class="nav nav-tabs" id="ownerTabs">
                            ...
                            <li class="nav-item">
                                <a class="nav-link {{ $tab == 'activity_logs' ? 'active' : '' }}"
                                    href="{{ route('company.realestate.owners.show', $owner->id) }}?tab=activity_logs">Activity
                                    Log</a>
                            </li>
                        </ul>
                        ...
                        <div class="tab-content p-4 border border-top-0 rounded-bottom">
                            @if ($tab == 'overview')
                                @include('company.realestate.owners.partials._overview', [
                                    'owner' => $owner,
                                ])
                            @elseif($tab == 'properties')
                                @include('company.realestate.owners.partials._properties', [
                                    'owner' => $owner,
                                ])
                            @elseif($tab == 'requests')
                                @include('company.realestate.owners.partials._requests', [
                                    'owner' => $owner,
                                ])
                            ...
                            @elseif($tab == 'documents')
                                @include('company.realestate.owners.partials._documnets', [
                                    'owner' => $owner,
                                ])
                            @elseif($tab == 'activity_logs')
                                @include('company.realestate.owners.partials._activity_logs', [
                                    'owner' => $owner,
                                ])
                            @endif
                        </div>
